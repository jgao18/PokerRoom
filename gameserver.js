...
function init() {
  ...
  io.on('connection', function(socket) {
    ...
    socket.emit('welcome', {
      message: latestPlayerUsername + " " + latestPlayerChipAmount
    });
    socket.on('linkUsername', retrieveUsername);
    socket.on('linkChipAmount', retrieveChipAmount);
    socket.on('disconnectLink', function(data) {
      util.log(this.id);
...
function amountChanged(data) {
  ...
  this.broadcast.emit("change amount", {
    username: data.id,
    chips: data.chips
  });
}
...
function potIncrease(data) {
  ...
  this.emit("add to pot", {
    chips: data.chips,
    amount: data.amount
  });
  this.broadcast.emit("add to pot", {
    chips: data.chips,
    amount: data.amount
  });
}
...
function onNewPlayer(data) {
  ...
  for (i = 0; i < connectedPlayers.length; i++) {
    ...
    outputArray[existingPlayer.getTableIndex()] = {
      id: existingPlayer.id,
      username: existingPlayer.getUsername(),
      chips: existingPlayer.getChips(),
      index: existingPlayer.getTableIndex()
    };
  }
...
function buttons(data) {
  ...
  for (i = 0; i < userSockets.length; i++) {
    if (playingPlayers[indexPlayer].getUsername() == userSockets[i].username) {
      ...
      this.emit("signal", {
        username: userSockets[i].username
      });
      this.broadcast.emit("signal", {
        username: userSockets[i].username
      });
...
function fold(data) {
  ...
  this.emit("remove folded cards", {
    user: data.username
  });
  this.broadcast.emit("remove folded cards", {
    user: data.username
  });
  ...
  if (playingPlayers.length == 1) {
    ...
    this.emit("remove buttons");
    this.broadcast.emit("remove buttons");
    this.emit("winning player", {
      player: user.getUsername()
    });
    this.broadcast.emit("winning player", {
      player: user.getUsername()
    });
    this.emit("round over", {
      status: "more than two players"
    });
...
function firstTurn(data) {
  ...
  if ((numTimesAccess == currentHandPlayers.length) && (playersReady > 1)) {
    ...
    userSocket.emit("signal", {
      username: userSockets[0].username
    });
...
function currentTurn(data) {
  ...
  if (currentHandPlayers.length == 0) {
    ...
    if (roundOver == false) {
      ...
      if (stage == "flop") {
        for (i = 0; i < userSockets.length; i++) {
          userSocket = userSockets[i].socket;
          userSocket.emit("flop cards", {
            value1: tableCards[0].get_value(),
            suit1: tableCards[0].get_suit(),
            value2: tableCards[1].get_value(),
            suit2: tableCards[1].get_suit(),
            value3: tableCards[2].get_value(),
            suit3: tableCards[2].get_suit()
          });
        }
      } else if (stage == "turn") {
        for (i = 0; i < userSockets.length; i++) {
          userSocket = userSockets[i].socket;
          userSocket.emit("turn card", {
            value: tableCards[3].get_value(),
            suit: tableCards[3].get_suit()
          });
        ...
        for (i = 0; i < userSockets.length; i++) {
          userSocket = userSockets[i].socket;
          userSocket.emit("river card", {
            value: tableCards[4].get_value(),
            suit: tableCards[4].get_suit()
          });
        ...
        for (var i = 0; i < usernames.length; i++) {
          playerHands[usernames[i]] = {
            "Card1": playerCards[times],
            "Card2": playerCards[times + 1]
          };
          times += 2;
        }
        ...
        for (var i = 0; i < playerCards.length; i++) {
          ...
          outputPlayerCards.push({
            value: playerCards[i].get_value(),
            suit: playerCards[i].get_suit(),
            owner: playerCards[i].get_owner()
          });
  ...
  if (data.action == "raise") {
    this.emit("player's action", {
      player: data.user,
      action: "raised",
      amount: data.amount
    });
    this.broadcast.emit("player's action", {
      player: data.user,
      action: "raised",
      amount: data.amount
    });
  ...
  currentHandPlayers.splice(0, 1);
  this.emit("current turn", {
    username: playerTurn.getUsername(),
    index: playerTurn.getTableIndex()
  });
  this.broadcast.emit("current turn", {
    username: playerTurn.getUsername(),
    index: playerTurn.getTableIndex()
  });
};
...
function onSocketDisconnect() {
  ...
  if (ip.address().indexOf("192.168.1.101") >= 0) {
    // Couldn't get this to pass in multiple values, so used the ~ (disallowed from usernames) as a seperator
    http.post('http://localhost/~gaoj/PokerRoom/php/update_user.php', {
      user_data: [username + "~" + chips]
    }, function(res) {
